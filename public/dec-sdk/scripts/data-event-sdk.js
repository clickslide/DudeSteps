function handleDeleteWithOptions(e, t, o, n, r) {
    ...
    for (attrKey in i) {
        ...
        var c = e + "." + attrKey,
            u = getCurrentObject(getComponent(c), getNameSpace(c));
...
function handleDeleteOfArrayTypes(e, t) {
    ...
    if (null != o) {
        ...
        for (var c = (new Array, []), u = 0, g = 0; g < n.length; g++)
            for (var d = 0; i > d; d++) {
                var p = l[d],
                    f = s[d],
                    v = n[g];
...
function handleDeleteWithNoOptions(e, t, o, n) {
    var r = "",
        a = getValueForHandle(t, o, n, e);
    if (t) console.log("Delete called at leaf level with no options"), r = localStorage.removeItem(e);
    else {
...
function contains(e) {
    for (var t = componentsArray.length; t--;)
        if (componentsArray[t] === e) return !0;
    return !1
}
...
function initialize(callbackFn, components, applicationId) {
    ...
    if (null == inValidComponent) {
        ...
        if ("span" == connectionType) console.log("Connection Type is Span. Initializing Web Socket"), RunSpanMessageBroker();
        else {
...
function showLocalStorage() {
    var e = "",
        t = "<table><tr><th>Name</th><th>Value</th></tr>",
        o = 0;
    ...
    if (null != decSdkSubscription) {
        ...
        for (var counter = 0; counter < subscriptionArray.length; counter++) {
            var subscriptionItem = subscriptionArray[counter],
                subCallback = subscriptionItem.callback,
                fn = eval("(" + subCallback + ")");
...
function getMatchingSubscriptions(e, t) {
    ...
    if (null != o) {
        ...
        for (var l = 0; l < a.length; l++) {
            var s = a[l],
                i = s.options;
            if (null == i) console.log("Options is null, adding subscription to the list"), s.value = t, n[r] = s, r++;
            else {
                for (key in i);
                var c = i[key];
...
function notifySpan(e) {
    ...
    var o = "ws://" + t.host + ":" + t.port + "/",
        n = new WebSocket(o),
        r = createDataHandle(e.component, e.namespace),
        a = e.method,
        l = getApplicationId(e.component);
...
function notifyDECCore(e) {
    ...
    var o = ["DECCore"],
        n = createDataHandle(e.component, e.namespace),
        r = e.method;
...
function showLocalStorageForSubscriptions() {
    var e = "",
        t = "<H3>Subscription Details Stored in local Storage for components</H3>\n<table><tr><th>Name</th><td><td><td><td></td></td></td></td><th>Value</th></tr>",
        o = 0;
    for (console.log("Local Storage length is" + localStorage.length), o = 0; o <= localStorage.length - 1; o++) {
        e = localStorage.key(o);
        var n = /\d/g;
...
function setItemsOfArrayType(e, t, o) {
    ...
    if (null === i || 0 === i.length) {
        if (console.log("Items Array is empty. Initialising and setting the values"), i = [], t instanceof Array)
            for (var c = i.length, u = 0; u < t.length; u++) {
                var g = t[u];
                i[c] = g, c++
            } else i[0] = t;
        i = uniqBy(i, JSON.stringify), localStorage.setItem(e, JSON.stringify(i)), r = !0, console.log("Array length " + i.length)
    } else if (null == o) {
        if (console.log("No Filter options. Adding the item to array"), t instanceof Array)
            for (var c = i.length, u = 0; u < t.length; u++) {
                var g = t[u];
                i[c] = g, c++
            } else i[i.length] = t;
...
function setItemsOfLeafOrNonLeafType(e, t, o) {
    ...
    var n = !1;
    console.log("Local workaround for zones as e = ", e);
    return n;
    if (e[t].isLeaf()) localStorage.setItem(e.getNodeLevel() + "." + t, o), n = !0;
    else
        for (key in o) {
...
function getValueForHandle(e, t, o, n) {
    ...
    for (var a = Object.keys(localStorage), r = (new Array, {}), l = 0, s = 0; s < a.length; s++)
        if (isNaN(parseInt(a[s])) && -1 != a[s].indexOf(n)) {
            var i = {},
                c = a[s].substring(a[s].lastIndexOf(".") + 1),
                u = getCurrentObject(getComponent(a[s]), getNameSpace(a[s]));
...
function executeSubscriptionCallbackAfterSet(handle, value) {
    var subscriptionHandle = createSubscriptionHandle(handle),
        subscriptionArray = getMatchingSubscriptions(subscriptionHandle, value);
    if (null == subscriptionArray || 0 == subscriptionArray.length) console.log("Component is not subscribed yet. So not sending subscription callback");
    else
        for (var counter = 0; counter < subscriptionArray.length; counter++) {
            var subscriptionItem = subscriptionArray[counter],
                subCallback = subscriptionItem.callback,
                subValue = subscriptionItem.value,
                fn = eval("(" + subCallback + ")");
...
function executeSubscriptionCallbackForParentChild(e, t) {
    ...
    for (var o = e.split("."), n = "", r = t, a = o.length - 1; a >= 0; a--)
        if (console.log("Printing Node Value:::" + o[a]), a == o.length - 1) executeSubscriptionCallbackAfterSet(e, t), n = e;
        else {
...
var Availability = {
    ...
    DecErrorCodes = {
        ...
        span_not_found: {
            errorcode: "span_server_not_connected",
            errormessage: "Not connected to span server, Please check the configuration"
        }
    },
    EventType = {
        ALL: "all",
    ...
    componentsArray = ["appmanager", "commerce", "connectivity", "identity", "media", "navigation", "notification", "policy", "sa", "search", "settings", "sms", "va", "vehicleinfo"],
    DecCommon = function () {
        ...
        this.setAppId = function (e) {
            ...
            if (r) console.log("Is already subscribed ::: true");
            else {
            ...
            if (console.log("Get Called for NodeLevel = " + this.nodeLevel + ", leaf = " + this.isLeaf()), t) {
                if (!this.isLeaf()) {
                    var n = Object.keys(localStorage),
                        r = (new Array, localStorage.getItem(this.nodeLevel)),
                        a = JSON.parse(r);
                    if (this.isArray) {
                        console.log("JSON: " + e);
                    ...
                    if (this.isMap) {
                        ...
                        if (o = "{", t instanceof Array)
                            for (var b = 0, N = 0; N < t.length; N++) 0 !== b && (o += ","), o += "'" + this.nodeLevel + "." + t[N] + "':'" + localStorage.getItem(this.nodeLevel + "." + t[N]) + "'", b++;
                        else
                            for (var b = 0, O = 0; O < n.length; O++) isNaN(parseInt(n[O])) && -1 != n[O].indexOf(this.nodeLevel) && (0 !== b && (o += ","), o += n[O] + ":'" + localStorage.getItem(n[O]) + "'", b++);
                        o += "}"
                    }
            ...
            try {
                ...
                var r = null;
                if (r = null == o ? EventType.ADD : EventType.UPDATE, !this.isLeaf() || this.isArray || this.isMap)
                    if (this.isArray) {
                    ...
                    for (p in u)
                        if (p = p, attrVal = u[p], console.log("ATTRIBUTE KEY::  " + p), console.log("ATTRIBUTE VALUE::  " + attrVal), this[p].isArray) {
                            var g = this.nodeLevel + "." + p,
                                l = setItemsOfArrayType(g, attrVal, o);
                            null != l && (i = !0)
                        } else i = setItemsOfLeafOrNonLeafType(this, p, attrVal);
                    if (i) {
                        var d = this.nodeLevel; - 1 == d.indexOf(".") && (d += ".");
                        var s = {
                            component: getComponent(d),
                    ...
                    var s = {
                        component: getComponent(d),
                        namespace: d.substring(d.indexOf(".") + 1, d.lastIndexOf(".")),
                        method: "set",
                        eventType: r,
                        value: a,
                        options: o
...
(function e(t, n, r) {
    ...
    4: [
        function (require, module, exports) {
            ...
            "/Users/forbeslindesay/GitHub/promisejs.org/node_modules/browserify/node_modules/insert-module-globals/node_modules/process/browser.js": 1
        }],
    5: [
        function (require, module, exports) {
...
function getProperties() {
    ...
    console.log("Properties are set:" + "Host is: " + host + " and Port is: " + port);
    return properties;
}
...
function RunSpanMessageBroker() {
    if ("WebSocket" in window) {
        ...
        ws.onopen = function () {
            ...
            var o = e.data;
            if (console.log("Received msg from span is ::: " + o), console.log("index of letrequest is::" + o.indexOf("dec.LetRequest")), -1 != o.indexOf("dec.LetRequest")) {
                var r = JSON.parse(o),
                    n = r["dec.LetRequest"];
                decodedData = decodeURIComponent(n.request.data), console.log("DecodedData::::" + decodedData);
                var t = JSON.parse(decodedData),
                    l = t.component,
                    s = t.nameSpace,
                    a = t.value,
                    c = t.options,
                    i = t.method,
                    d = t.eventType;
                console.log("component is:::: " + l), console.log("namespace is:::: " + s), console.log("value is:::: " + a), console.log("options is:::: " + c), console.log("method is:::: " + i), console.log("eventType is:::: " + d);
                var u = "";
                u = null == s || 0 == s.length ? l : l + "." + s;
                var p = getCurrentObject(l, s);
                if (null != i && "clear" === i) {
                    var f = c || null;
                    console.log("Received Delete method call for NodeLevel = " + u + ", leaf = " + p.isLeaf());
                    var g = null;
                    f ? (g = handleDeleteWithOptions(u, c, p.isLeaf(), p.isArray, p.isMap), console.log("Deleted Item ::: ", g)) : (g = handleDeleteWithNoOptions(u, p.isLeaf(), p.isArray, p.isMap), console.log("Deleted Item ::: ", g)), null == a && (a = g)
                } else if (p.isArray && null != a) console.log("Received component is of Array Type"), setItemsOfArrayType(u, a, c);
                else {
                    console.log("Received component is of non Array Type");
                    for (attrKey in a) {
                        attrVal = a[attrKey], console.log("ATTRIBUTE KEY::  " + attrKey), console.log("ATTRIBUTE VALUE::  " + attrVal);
                        var v = "";
                        v = null == s || "" == s ? attrKey : s + "." + attrKey;
                        var y = getCurrentObject(l, v);
                        if (null != y)
                            if (y.isArray) {
                                var b = u + "." + attrKey;
                                setItemsOfArrayType(b, attrVal, c)
                            } else setItemsOfLeafOrNonLeafType(p, attrKey, attrVal);
                        else console.log("Unable to get the current object for " + l + "." + v)
                    }
                }
                executeSubscriptionCallbackForParentChild(u, a)
            } else console.log("Received Set dec request..ignoring...")
        }, ws.onclose = function () {
            console.log("Connection is closed...")
        }, ws.onerror = function (err) {
            console.log("websocket connection to host " + properties.host + " port " + properties.port + " failed"), console.log("error is::" + err);
...
function getHashCode(e) {
    ...
    for (i = 0; i < e.length; i++) char = e.charCodeAt(i), o = (o << 5) - o + char, o &= o;
    return o
}

function createSubscriptionHandle(e) {
    var o = getHashCode(e) + e;
    return o
}

function createDataHandle(e, o) {
    var r = e + "." + o;
    return r
}

function getCurrentObject(e, o) {
    var r = drive[e];
    if (null == o || 0 == o.length) return r;
    if (-1 == o.indexOf(".")) return r[o];
    for (var n = o.split("."), t = 0; t < n.length; t++) {
        var l = n[t];
        r = r[l]
    }
    return r
